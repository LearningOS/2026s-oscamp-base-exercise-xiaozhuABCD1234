...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        // 先处理 level 2 和 level 1（中间节点）
        for level in [2, 1] {
            let vpn_idx = Self::extract_vpn(va, level);
            let pte = self.nodes.get(&current_ppn).unwrap().entries[vpn_idx];
            if (pte & PTE_V) == 0 {
                // 这条路还没修，新建一个页表节点
                let new_ppn = self.alloc_node();
                // 在当前 PTE 里写入：指向新节点的指针 + V 标志
                self.nodes.get_mut(&current_ppn).unwrap().entries[vpn_idx] =
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for level in [2, 1, 0] {
            ...
            if (pte & (PTE_R | PTE_W | PTE_X)) != 0 {
                ...
                let offest_mask = if level == 1 {
                    (1 << 21) - 1
                } else {
                    (1 << 12) - 1
                };
                let pa = (ppn << 12) | (va & offest_mask);
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let pte = self.nodes.get_mut(&self.root_ppn).unwrap().entries[vpn2];
        ...
        let node = self.nodes.get_mut(&mid_ppn).unwrap();
        let ppn = pa >> 12;
