...
pub fn sys_write(fd: usize, buf: &[u8]) -> isize {
    ...
    unsafe {
        syscall3(
            NATIVE_SYS_WRITE,      // syscall 号（条件编译选择）
            fd,                    // arg0: 文件描述符
            buf.as_ptr() as usize, // arg1: 缓冲区指针
            buf.len(),             // arg2: 长度
        )
    }
}
...
pub fn sys_read(fd: usize, buf: &mut [u8]) -> isize {
    ...
    unsafe {
        syscall3(
            NATIVE_SYS_READ,
            fd,
            buf.as_mut_ptr() as usize, // ⚠️ 注意是可变指针！
            buf.len(),
        )
    }
}
...
pub fn sys_exit(code: i32) -> ! {
    ...
    unsafe {
        ...
        // 🎯 syscall 成功后不会返回，但如果失败了（理论上不会）:
        // 用 unreachable!() 告诉编译器"这里不会执行到"
